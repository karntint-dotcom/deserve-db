...
def load_data(file_bytes):
    ...
    # Auto-detect data sheet: ใช้ sheet แรกที่ชื่อขึ้นต้นด้วย "ข้อมูล" หรือ sheet แรกสุด
    xl = pd.ExcelFile(buf)
    sheet_names = xl.sheet_names
    data_sheet = sheet_names[0]   # default = sheet แรก
    for s in sheet_names:
        if s.startswith('ข้อมูล'):
            data_sheet = s; break

    buf.seek(0)
    df  = pd.read_excel(buf, sheet_name=data_sheet, header=1)

    # เงื่อนไข sheets — ลองหาตามชื่อ หรือใช้ sheet 2/3 ถ้าไม่เจอ
    buf.seek(0)
    c1_sheet = next((s for s in sheet_names if 'เงื่อนไข1' in s or s=='เงื่อนไข1'), sheet_names[1] if len(sheet_names)>1 else sheet_names[0])
    c2_sheet = next((s for s in sheet_names if 'เงื่อนไข2' in s or s=='เงื่อนไข2'), sheet_names[2] if len(sheet_names)>2 else sheet_names[0])
...
def build_dashboard(file_bytes, plc_target=1_250_000, plc_deadline="30 มิถุนายน 2569",
    ...
    if monthly_targets:
        upd(97, "🎯 เป้าหมายรายเดือน..."); build_sheet12(wb, df, grand_total, months, monthly_targets)
